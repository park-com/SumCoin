...
#include "soomcoinbridge.moc"

SoomcoinBridge::SoomcoinBridge(SoomcoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void SoomcoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void SoomcoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void SoomcoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void SoomcoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void SoomcoinBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void SoomcoinBridge::populateOptions()
{
...
// Transactions
bool SoomcoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void SoomcoinBridge::clearRecipients()
{
    recipients.clear();
}

bool SoomcoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 SOOM, 1 Soomcoin
        switch(rcp.txnTypeInd)
        {
            case TXT_SOOM_TO_SOOM:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SOOM, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_SOOM_TO_ANON:
                formatted.append(tr("<b>%1</b> to SOOMCOIN %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SOOM, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> SOOMCOIN, ring size %2 to SOOMCOIN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SOOM, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_SOOM:
                formatted.append(tr("<b>%1</b> SOOMCOIN, ring size %2 to SOOM %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SOOM, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SOOM, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsSoomcoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your soomcoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SOOM, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void SoomcoinBridge::openCoinControl()
{
...
void SoomcoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void SoomcoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap SoomcoinBridge::listAnonOutputs()
{
...
void SoomcoinBridge::populateTransactionTable()
{
...
void SoomcoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void SoomcoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString SoomcoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void SoomcoinBridge::populateAddressTable()
{
...
void SoomcoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void SoomcoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString SoomcoinBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString SoomcoinBridge::lastAddressError()
{
    QString sError;
...
QString SoomcoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void SoomcoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool SoomcoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool SoomcoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void SoomcoinBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void SoomcoinBridge::appendMessage(int row)
{
...
void SoomcoinBridge::populateMessageTable()
{
...
void SoomcoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool SoomcoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool SoomcoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString SoomcoinBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString SoomcoinBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool SoomcoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool SoomcoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString SoomcoinBridge::createGroupChat(QString label)
{
...
QString SoomcoinBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList SoomcoinBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void SoomcoinBridge::connectSignals()
{
...
QString SoomcoinBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap SoomcoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap SoomcoinBridge::listLatestBlocks()
{
...
QVariantMap SoomcoinBridge::findBlock(QString searchID)
{
...
QVariantMap SoomcoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap SoomcoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap SoomcoinBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap SoomcoinBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap SoomcoinBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap SoomcoinBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap SoomcoinBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap SoomcoinBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap SoomcoinBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap SoomcoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Soomcoincoin prefix.");
            return result;
        }
...
QVariantMap SoomcoinBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap SoomcoinBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap SoomcoinBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
