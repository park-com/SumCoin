...
SumcoinGUI::SumcoinGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new SumcoinBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Sumcoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/sumcoin"));
    setWindowIcon(QIcon(":icons/sumcoin"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/sumcoin/index.html");
#else
    QFile html("/opt/sumcoin/index.html");
#endif
...
void SumcoinGUI::pageLoaded(bool ok)
{
...
void SumcoinGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void SumcoinGUI::urlClicked(const QUrl & link)
{
...
void SumcoinGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/sumcoin"), tr("&About Sumcoin"), this);
    ...
    toggleHideAction = new QAction(QIcon(":/icons/sumcoin"), tr("&Show / Hide"), this);
...
void SumcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Sumcoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/sumcoin_testnet"));
            setWindowIcon(QIcon(":icons/sumcoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/sumcoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Sumcoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/sumcoin_testnet"));
...
void SumcoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void SumcoinGUI::setMessageModel(MessageModel *messageModel)
{
...
void SumcoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setIcon(QIcon(":/icons/sumcoin"));
...
#ifndef Q_OS_MAC
void SumcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SumcoinGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to Sumcoin network", "", count));
}

void SumcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void SumcoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void SumcoinGUI::changeEvent(QEvent *e)
{
...
void SumcoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void SumcoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::SUM, nFeeRequired));
...
void SumcoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void SumcoinGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void SumcoinGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void SumcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SumcoinGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void SumcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void SumcoinGUI::handleURI(QString strURI)
{
...
void SumcoinGUI::setEncryptionStatus(int status)
{
...
void SumcoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void SumcoinGUI::changePassphrase()
{
...
void SumcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void SumcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SumcoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void SumcoinGUI::updateStakingIcon()
{
...
void SumcoinGUI::detectShutdown()
{
