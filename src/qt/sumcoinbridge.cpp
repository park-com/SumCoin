...
#include "sumcoinbridge.moc"

SumcoinBridge::SumcoinBridge(SumcoinGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void SumcoinBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void SumcoinBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void SumcoinBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void SumcoinBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void SumcoinBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void SumcoinBridge::populateOptions()
{
...
// Transactions
bool SumcoinBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void SumcoinBridge::clearRecipients()
{
    recipients.clear();
}

bool SumcoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 SUM, 1 Sumcoin
        switch(rcp.txnTypeInd)
        {
            case TXT_SUM_TO_SUM:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SUM, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_SUM_TO_ANON:
                formatted.append(tr("<b>%1</b> to SUMCOIN %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SUM, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> SUMCOIN, ring size %2 to SUMCOIN %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SUM, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_SUM:
                formatted.append(tr("<b>%1</b> SUMCOIN, ring size %2 to SUM %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SUM, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SUM, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsSumcoinBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your sumcoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::SUM, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void SumcoinBridge::openCoinControl()
{
...
void SumcoinBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void SumcoinBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap SumcoinBridge::listAnonOutputs()
{
...
void SumcoinBridge::populateTransactionTable()
{
...
void SumcoinBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void SumcoinBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString SumcoinBridge::transactionDetails(QString txid)
{
...
// Addresses
void SumcoinBridge::populateAddressTable()
{
...
void SumcoinBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void SumcoinBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString SumcoinBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString SumcoinBridge::lastAddressError()
{
    QString sError;
...
QString SumcoinBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void SumcoinBridge::updateAddressLabel(QString address, QString label)
{
...
bool SumcoinBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool SumcoinBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void SumcoinBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void SumcoinBridge::appendMessage(int row)
{
...
void SumcoinBridge::populateMessageTable()
{
...
void SumcoinBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool SumcoinBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool SumcoinBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString SumcoinBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString SumcoinBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool SumcoinBridge::setPubKey(QString address, QString pubkey)
{
...
bool SumcoinBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString SumcoinBridge::createGroupChat(QString label)
{
...
QString SumcoinBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList SumcoinBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void SumcoinBridge::connectSignals()
{
...
QString SumcoinBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap SumcoinBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap SumcoinBridge::listLatestBlocks()
{
...
QVariantMap SumcoinBridge::findBlock(QString searchID)
{
...
QVariantMap SumcoinBridge::blockDetails(QString blkHash)
{
...
QVariantMap SumcoinBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap SumcoinBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap SumcoinBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap SumcoinBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap SumcoinBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap SumcoinBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap SumcoinBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap SumcoinBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap SumcoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Sumcoincoin prefix.");
            return result;
        }
...
QVariantMap SumcoinBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap SumcoinBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap SumcoinBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
