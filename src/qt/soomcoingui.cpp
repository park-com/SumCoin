...
SoomcoinGUI::SoomcoinGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new SoomcoinBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Soomcoin") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/soomcoin"));
    setWindowIcon(QIcon(":icons/soomcoin"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/soomcoin/index.html");
#else
    QFile html("/opt/soomcoin/index.html");
#endif
...
void SoomcoinGUI::pageLoaded(bool ok)
{
...
void SoomcoinGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void SoomcoinGUI::urlClicked(const QUrl & link)
{
...
void SoomcoinGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/soomcoin"), tr("&About Soomcoin"), this);
    ...
    toggleHideAction = new QAction(QIcon(":/icons/soomcoin"), tr("&Show / Hide"), this);
...
void SoomcoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void SoomcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("Soomcoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/soomcoin_testnet"));
            setWindowIcon(QIcon(":icons/soomcoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/soomcoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Soomcoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/soomcoin_testnet"));
...
void SoomcoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void SoomcoinGUI::setMessageModel(MessageModel *messageModel)
{
...
void SoomcoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setIcon(QIcon(":/icons/soomcoin"));
...
#ifndef Q_OS_MAC
void SoomcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SoomcoinGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to Soomcoin network", "", count));
}

void SoomcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void SoomcoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void SoomcoinGUI::changeEvent(QEvent *e)
{
...
void SoomcoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void SoomcoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::SOOM, nFeeRequired));
...
void SoomcoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void SoomcoinGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void SoomcoinGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void SoomcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SoomcoinGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void SoomcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void SoomcoinGUI::handleURI(QString strURI)
{
...
void SoomcoinGUI::setEncryptionStatus(int status)
{
...
void SoomcoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void SoomcoinGUI::changePassphrase()
{
...
void SoomcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void SoomcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SoomcoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void SoomcoinGUI::updateStakingIcon()
{
...
void SoomcoinGUI::detectShutdown()
{
